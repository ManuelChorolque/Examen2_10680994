...
namespace SistemasVentas.DAL
{
    public class DetalleIngDal
    {
        ...
        {
            string consulta = "SELECT D.IDPRODUCTO, P.NOMBRE, D.FECHAVENC " +
                             "FROM DETALLEING D INNER JOIN PRODUCTO P ON D.IDPRODUCTO = P.IDPRODUCTO " +
                            "WHERE D.FECHAVENC >= GETDATE() AND D.FECHAVENC <= DATEADD(day, 30, GETDATE())";
